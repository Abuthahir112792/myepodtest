...
if(isset($_POST['tempValues']))
{
	...
	if ($plan == 4) {
      ...
      $plan_duration = '6 Months';
		...
		$sqlQueryupdate = "UPDATE users SET plan_id = '".$plan."' , plan_name = '".$plan_name."' ,renewal_status = 'C', plan_start = '".$current_date."', plan_expire = '".$end_date."', renewal_date = '' WHERE userid = '".$arrval."' ";
	...
	plan_start = '".$current_date."' AND plan_end = '".$end_date."' AND plan_id= '".$plan."' AND status = 'Plan Expired Renewed Email' AND type= 'Receipt'"));	
	...
	if($dupQry1 == 0){
		...
		plan_id= '".$plan."', email = '".$email."', status = 'Plan Expired Renewed Email' , type= 'Receipt' , invoiceno = '".$invoiceno."'  ";
	...
	$clientsubject = "Expired Account Renewed - Receipt No:  $invoiceno";
...
$canvas->text($x, $y, $text, $font, 75); 
	...
	$i++;		
	 ...
	 echo "<script> window.location.href = 'planexpired.php?msg=Y';</script>";
